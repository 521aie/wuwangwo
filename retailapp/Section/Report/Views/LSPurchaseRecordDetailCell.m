...
@interface LSPurchaseRecordDetailCell ()
/** 背景 */
...
@implementation LSPurchaseRecordDetailCell
+ (instancetype)purchaseRecordDetailCellWithTableView:(UITableView *)tableView{
    static NSString *cellIdentifier = @"cell";
    LSPurchaseRecordDetailCell *cell = [tableView dequeueReusableCellWithIdentifier:cellIdentifier];
    if (cell == nil) {
        cell = [[LSPurchaseRecordDetailCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellIdentifier];
...
- (void)configViews {
    ...
    self.lblGoodsName.lineBreakMode = NSLineBreakByWordWrapping;
...
- (void)configConstraints {
    ...
    [viewLeftLine makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.left.left.equalTo(wself.backView.left);
    ...
    [viewRightLine makeConstraints:^(MASConstraintMaker *make) {
        make.top.bottom.equalTo(wself.backView);
        make.right.equalTo(wself.backView.right);
        make.width.equalTo(1);
    }];
