//
//  LSGoodsTransactionFlowDetailController.m
//  retailapp
//
...
@interface LSGoodsTransactionFlowDetailController ()<UITableViewDelegate, UITableViewDataSource>
/** 表格 */
...
@implementation LSGoodsTransactionFlowDetailController
...
- (void)configViews {
    ...
    //标题
    [self configTitle:@"交易流水" leftPath:Head_ICON_BACK rightPath:nil];
    //表格
...
- (void)loadData {
    ...
    NSString *url = @"orderDetailsReport/v2/getDetail";
    [BaseService getRemoteLSDataWithUrl:url param:self.param withMessage:nil show:YES CompletionHandler:^(id json) {
        ...
        if ([ObjectUtil isNotNull:orderReportVo]) {
            ...
            //销售时间 订单类型 后台不给需要从上一个页面获取
            wself.orderReportVo.salesTime = wself.salesTime;
            wself.orderReportVo.orderKind = wself.orderKind;
            //YES 是销售单  NO是退货单这个值要在表格刷新的前面 因为表格刷新的时候需要这个值
            wself.isOrder =  wself.orderKind;
