...
+ (id)getInstance{
    EmployeeCell *item = [[[NSBundle mainBundle]loadNibNamed:@"EmployeeCell" owner:self options:nil]lastObject];
    return item;
}
    ...
    if ([obj isKindOfClass:[EmlpoyeeUserIntroVo class]]) {
        ...
        if([NSString isNotBlank:userIntro.fileName]){
            ...
            if (userIntro.sex == 2) {//女
                [self.imgHead setImage:[UIImage imageNamed:@"img_employee_cellbg_female"]];
            }else{//男
                [self.imgHead setImage:[UIImage imageNamed:@"img_employee_cellbg_male"]];
            }

        }
    }
    //UserPerformanceTargetVo
    if ([obj isKindOfClass:[UserPerformanceTargetVo class]]) {
        UserPerformanceTargetVo *userIntro = (UserPerformanceTargetVo*)obj;
        self.lblName.text = userIntro.name;
        self.lblDetail1.text = [NSString stringWithFormat:@"目标:%@",userIntro.totalTarget];
        if ([ObjectUtil isNotNull:userIntro.mobile]  && ![userIntro.mobile isEqualToString:@""]) {
            self.lblDetail2.text = [NSString stringWithFormat:@"手机:%@",userIntro.mobile];
        }else{
            self.lblDetail2.text = [NSString stringWithFormat:@"手机:%@",@"无"];
        ...
        self.lblRole.text = @"";
        if([NSString isNotBlank:userIntro.filePath]){
            [self.imgHead sd_setImageWithURL:[NSURL URLWithString:[NSString urlFilterRan:userIntro.filePath]] placeholderImage:nil];
