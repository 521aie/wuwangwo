...
+ (instancetype)smsRetainNumItem {
    ...
    item.frame = CGRectMake(0, 0, SCREEN_W, 48);
...
- (void)showChargeBtn:(BOOL)isShow {
    ...
    __weak typeof(self) wself = self;
    [self.lblVal updateConstraints:^(MASConstraintMaker *make) {
        make.right.equalTo(isShow ? wself.btn.left : wself.right).offset(-10);
    }];
}
