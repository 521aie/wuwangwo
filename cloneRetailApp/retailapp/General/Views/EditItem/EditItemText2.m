...
- (void)awakeFromNib {
    ...
    self.view.frame = CGRectMake(0, 0, SCREEN_W, 48);
...
+ (instancetype)editItemText {
    EditItemText2 *view = [[EditItemText2 alloc] initWithFrame:CGRectMake(0, 0, SCREEN_W, 48)];
    [view awakeFromNib];
    return view;
}
...
- (void)initPosition:(NSUInteger)num {
    ...
    if (num == 0) {
        ...
        [self layoutIfNeeded];
        [self.txtVal updateConstraints:^(MASConstraintMaker *make) {
             make.right.equalTo(wself.btnButton.left).offset(self.btnButton.ls_width);
        }];
    }else{
        [self.btnButton setHidden:NO];
        [self.txtVal updateConstraints:^(MASConstraintMaker *make) {
            make.right.equalTo(wself.btnButton.left).offset(0);
        }];
        [self.btnButton updateConstraints:^(MASConstraintMaker *make) {
