//
//  LSPaymentOrderDetailHeaderView.m
//  retailapp
//
...
#import "DateUtils.h"
@interface LSPaymentOrderDetailHeaderView()
/** 会员View */
@property (nonatomic, strong) LSPaymentOrderDetailMemberView *viewMember;
/** 会员View高度 */
...
@end
@implementation LSPaymentOrderDetailHeaderView
+ (instancetype)paymentOrderDetailHeaderView {
    LSPaymentOrderDetailHeaderView *view = [[LSPaymentOrderDetailHeaderView alloc] initWithFrame:CGRectMake(0, 0, SCREEN_W, 100)];
    [view configViews];
...
- (void)configViews {
    ...
    //会员View
    self.viewMember = [LSPaymentOrderDetailMemberView paymentOrderDetailMemberView];
...
- (void)setOrderInfoVo:(OrderInfoVo *)orderInfoVo memberInfo:(LSPersonDetailVO *)memberInfo orderCode:(NSString *)orderCode {
    if ([NSString isNotBlank:memberInfo.payName]) {
        //设置会员信息
        self.viewMember.personDetailVO = memberInfo;
    } else {
        __weak typeof(self) wself = self;
    ...
    NSString *code = [NSString shortStringForOrderID:orderCode];
    ...
    //订单时间
    //支付时间
    NSString *txtTime = nil;
    if (orderInfoVo.payDate == 0) {
        txtTime = @"";
    } else {
        txtTime = [DateUtils formateTime:orderInfoVo.payDate];
    }
    self.lblOrderTime.text = txtTime;
    //强制布局
    [self layoutIfNeeded];
    self.ls_height = self.viewBg.ls_bottom;
