...
@interface LSPurchaseRecordHeaderView()
/** 供应商 */
...
@implementation LSPurchaseRecordHeaderView
+ (instancetype)purchaseRecordHeaderView  : (LSGoodsPurchaseVo *)obj{
    LSPurchaseRecordHeaderView *view = [[LSPurchaseRecordHeaderView alloc] initWithFrame:CGRectMake(0, 0, SCREEN_W, 119)];
    [view configViews : obj];
    return view;
}

- (void)configViews : (LSGoodsPurchaseVo *)obj {
    ...
    [self.lblSuppiler makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.left.equalTo(wself.right).offset(-margin);
    }];
