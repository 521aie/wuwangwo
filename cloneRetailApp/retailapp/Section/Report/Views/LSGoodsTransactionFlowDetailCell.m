...
/** 计次标志 */
@property (nonatomic, strong) UILabel *lblMeterStatus;
/** 商品对象 */
...
- (void)configViews {
    ...
    [self.contentView addSubview:self.lblGoodsCode];
    self.lblGoodsCode.numberOfLines = 0;
    ...
    //计次标志
    self.lblMeterStatus = [[UILabel alloc] init];
    self.lblMeterStatus.font = [UIFont systemFontOfSize:10];
    self.lblMeterStatus.textColor = [UIColor whiteColor];
    self.lblMeterStatus.backgroundColor = RGB(1, 178, 43);
    self.lblMeterStatus.layer.cornerRadius = 4;
    self.lblMeterStatus.layer.masksToBounds = YES;
    self.lblMeterStatus.textAlignment = NSTextAlignmentCenter;
    [self.lblMeterStatus setHidden:YES];
    [self.contentView addSubview:self.lblMeterStatus];
}
...
- (void)setGoodsInfo:(LSOrderDetailReportVo *)goodsInfo isOrder:(BOOL)isOrder {
    ...
    //计次标志
    [self.lblMeterStatus remakeConstraints:^(MASConstraintMaker *make) {
        make.size.equalTo(15);
        make.top.equalTo(wself.lblGoodsAmmount);
        make.left.equalTo(wself.viewLeftLine.right).offset(5);
    }];
...
//        }
    ...
    if (goodsInfo.ratio.intValue == 100) {
        ...
        [self.lblStatus setHidden:NO];
        self.lblStatus.backgroundColor = RGB(255, 179, 0);
    } else if (10 == [goodsInfo.discountType integerValue]) {
        self.lblMeterStatus.text = @"次";
        [self.lblStatus setHidden:YES];
        [self.lblMeterStatus setHidden:NO];
        self.lblMeterStatus.backgroundColor = RGB(255, 152, 0);
        
        [self.lblGoodsName remakeConstraints:^(MASConstraintMaker *make) {
            make.right.equalTo(wself.viewCenterLeftLine.left);
            make.top.equalTo(wself.lblGoodsAmmount);
            make.left.equalTo(wself.lblMeterStatus.right).offset(5);
        }];
        
        [self.lblGoodsCode remakeConstraints:^(MASConstraintMaker *make) {
            make.right.equalTo(wself.viewCenterLeftLine.left);
            make.top.equalTo(wself.lblGoodsName.bottom);
            make.left.equalTo(wself.lblMeterStatus.left);
        }];
    }
    
    CGFloat maxHeight = MAX((self.lblGoodsName.ls_height + self.lblGoodsCode.ls_height), self.lblGoodsAmmount.ls_height);
